...
describe('resource communities', () => {
  ...
  test.skip('delete: only required params', async () => {
    const responsePromise = client.x.communities.delete('id', { account: '@elonmusk', community_name: 'Tesla Fans' });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
  ...
  test.skip('delete: required and optional params', async () => {
    const response = await client.x.communities.delete('id', { account: '@elonmusk', community_name: 'Tesla Fans' });
  });
  ...
  test.skip('retrieveMembers: request options and params are passed correctly', async () => {
    ...
    await expect(client.x.communities.retrieveMembers('id', { cursor: 'cursor', pageSize: 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test.skip('retrieveModerators: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.x.communities.retrieveModerators('id', { cursor: 'cursor' }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(XTwitterScraper.NotFoundError);
  });
