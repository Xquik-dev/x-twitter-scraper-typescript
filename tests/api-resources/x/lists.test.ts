...
describe('resource lists', () => {
  ...
  test.skip('retrieveFollowers: request options and params are passed correctly', async () => {
    ...
    await expect(client.x.lists.retrieveFollowers('id', { cursor: 'cursor' }, { path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test.skip('retrieveMembers: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.x.lists.retrieveMembers('id', { cursor: 'cursor', pageSize: 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(XTwitterScraper.NotFoundError);
  });
  ...
  test.skip('retrieveTweets: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.x.lists.retrieveTweets('id', {
    cursor: 'cursor',
    includeReplies: true,
    sinceTime: 'sinceTime',
    untilTime: 'untilTime',
