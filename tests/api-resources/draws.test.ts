...
describe('resource draws', () => {
  ...
  test.skip('list: request options and params are passed correctly', async () => {
    ...
    await expect(client.draws.list({ after: 'after', limit: 1 }, { path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test.skip('export: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.draws.export('id', { format: 'csv', type: 'winners' }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(XTwitterScraper.NotFoundError);
  });
