...
describe('resource extractions', () => {
  ...
  test.skip('retrieve: request options and params are passed correctly', async () => {
    ...
    await expect(client.extractions.retrieve('id', { after: 'after', limit: 1 }, { path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test.skip('list: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.extractions.list({
    after: 'after',
    limit: 1,
    status: 'running',
  ...
  test.skip('exportResults: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.extractions.exportResults('id', { format: 'csv' }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(XTwitterScraper.NotFoundError);
  });
