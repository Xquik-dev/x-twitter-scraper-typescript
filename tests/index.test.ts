...
import { APIPromise } from 'x-twitter-scraper/core/api-promise';

import util from 'node:util';
import XTwitterScraper from 'x-twitter-scraper';
import { APIUserAbortError } from 'x-twitter-scraper';
...
describe('logging', () => {
  ...
  beforeEach(() => {
    process.env = { ...env };
    process.env['X_TWITTER_SCRAPER_LOG'] = undefined;
  });
  ...
  const forceAPIResponseForClient = async (client: XTwitterScraper) => {
  ...
  test('default logLevel is warn', async () => {
    const client = new XTwitterScraper({ apiKey: 'My API Key' });
  ...
  test('debug logs happen with debug env var', async () => {
    ...
    process.env['X_TWITTER_SCRAPER_LOG'] = 'debug';
    const client = new XTwitterScraper({ logger: logger, apiKey: 'My API Key' });
  ...
  test('warn when env var level is invalid', async () => {
    ...
    process.env['X_TWITTER_SCRAPER_LOG'] = 'not a log level';
    const client = new XTwitterScraper({ logger: logger, apiKey: 'My API Key' });
    expect(client.logLevel).toBe('warn');
    expect(warnMock).toHaveBeenCalledWith('process.env[\'X_TWITTER_SCRAPER_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]');
  });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new XTwitterScraper({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['X_TWITTER_SCRAPER_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new XTwitterScraper({ baseURL: 'https://example.com', apiKey: 'My API Key' });
    ...
    test('in request options overridden by client options', () => {
      const client = new XTwitterScraper({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
  ...
  test('maxRetries option is correctly set', () => {
    const client = new XTwitterScraper({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new XTwitterScraper({ apiKey: 'My API Key' });
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new XTwitterScraper({ apiKey: 'My API Key', fetch: testFetch });
